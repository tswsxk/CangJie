...
class WRCLSTM(gluon.HybridBlock):
    ...
    def hybrid_forward(self, F, w, rw, c, rc,
        ...
        if self.net_type == "bilstm_att":
            ...
            attention = F.concat(
                w_lo, w_ro, wr_lo, wr_ro, c_lo, c_ro
            )
            ...
            attention = F.concat(w_o, wr_o, c_o)
        else:
            raise TypeError(
                "net_type should be lstm, bilstm or bilstm_att,"
                " now is %s" % self.net_type
